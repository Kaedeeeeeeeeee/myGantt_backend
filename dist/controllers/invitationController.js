import { createInvitation, acceptInvitation, rejectInvitation, cancelInvitation, getProjectInvitations, getUserPendingInvitations, getInvitationByToken, ProjectRole, } from '../services/invitationService.js';
...
export const createInvitationHandler = async (req, res, next) => {
    try {
        ...
        const invitation = await createInvitation(projectId, req.user.userId, {
 ...
 */
export const getInvitationHandler = async (req, res, next) => {
    try {
 ...
 */
export const rejectInvitationHandler = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('User not authenticated', 401);
        }
 ...
 */
export const cancelInvitationHandler = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('User not authenticated', 401);
        }
 ...
 */
export const getUserPendingInvitationsHandler = async (req, res, next) => {
    try {
        if (!req.user) {
            throw new AppError('User not authenticated', 401);
        }
