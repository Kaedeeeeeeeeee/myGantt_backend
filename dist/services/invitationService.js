 ...
 * 创建项目邀请
 */
export const createInvitation = async (projectId, inviterId, data) => {
    // 1. 验证项目存在且邀请者有权限
 ...
 * 拒绝邀请
 */
export const rejectInvitation = async (token, userId) => {
    // 1. 获取邀请详情
    const invitation = await getInvitationByToken(token);
    // 2. 验证用户邮箱匹配
    const user = await prisma.user.findUnique({
 ...
 * 获取项目的所有邀请
 */
export const getProjectInvitations = async (projectId, userId) => {
    // 验证用户有权限查看邀请
    const project = await prisma.project.findFirst({
        where: {
            id: projectId,
