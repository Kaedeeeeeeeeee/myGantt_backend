 ...
 * 获取用户在项目中的角色
 */
export const getUserProjectRole = async (projectId, userId) => {
 ...
 * 更新成员权限
 */
export const updateMemberRole = async (projectId, targetUserId, updaterUserId, data) => {
    // 验证项目存在
    const project = await prisma.project.findUnique({
        where: { id: projectId },
    });
 ...
 * 移除成员
 */
export const removeMember = async (projectId, targetUserId, removerUserId) => {
    // 验证项目存在
    const project = await prisma.project.findUnique({
        where: { id: projectId },
    });
    ...
    if (existingMember.role === ProjectRole.ADMIN &&
        ...
        throw new AppError('Cannot remove the last admin. Please promote another member to admin first.', 400);
    }
    // 移除成员
